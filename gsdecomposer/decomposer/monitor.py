import argparse
import os
import pickle

import matplotlib.pyplot as plt
import numpy as np
import torch

from gsdecomposer.plot_base import *
from gsdecomposer.udm.dataset import UDMDataset
from gsdecomposer.udm.loess import TRAIN_SECTIONS
from gsdecomposer.gan.dataset import GANDataset
from gsdecomposer.decomposer.model import Decomposer
from gsdecomposer.decomposer.train import ROOT_DIR


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--facies", type=str, default="loess",
                        help="the sedimentary facies to check")
    parser.add_argument("--experiment_id", type=int, default=1,
                        help="the id of this experiment")
    parser.add_argument("--batch", type=int, default=-1, help="the target batch to check")
    opt = parser.parse_args()

    checkpoint_dir = os.path.join(ROOT_DIR, opt.facies, str(opt.experiment_id), "checkpoints")
    udm_dataset_path = "./datasets/udm/loess/all_datasets.dump"
    cmap = plt.get_cmap("tab10")
    # plt.ion()
    plt.figure(figsize=(6.6, 3.7125))
    frames_dir = "./tmp/video_2_frames"
    frame_index = 0
    dataset = UDMDataset(udm_dataset_path, sections=TRAIN_SECTIONS, true_gsd=True)
    dataloader = torch.utils.data.DataLoader(dataset, 64, shuffle=True, drop_last=True)
    while True:
        batches = [int(os.path.splitext(filename)[0]) for filename in os.listdir(checkpoint_dir)]
        batches.sort()
        if opt.batch in batches:
            target_batch = opt.batch
        else:
            target_batch = batches[-2]
        decomposer_path = os.path.join(checkpoint_dir, f"{target_batch}.pkl")
        checkpoint = torch.load(decomposer_path, map_location="cpu")
        decomposer = checkpoint["decomposer"]
        GSDs_to_decompose = next(iter(dataloader))[0]
        proportions, components = decomposer(GSDs_to_decompose)
        GSDs = torch.squeeze(proportions @ components, dim=1)
        GSDs_to_decompose = GSDs_to_decompose.detach().cpu().numpy()
        GSDs = GSDs.detach().cpu().numpy()
        proportions = proportions.detach().cpu().numpy()
        components = components.detach().cpu().numpy()
        plt.clf()
        title = f"[Facies - {opt.facies.replace('_', ' ').capitalize()}] " \
                f"[Experiment #{opt.experiment_id}] " \
                f"[Batch - {target_batch}]"
        plt.gcf().canvas.manager.window.setWindowTitle(title)
        for i in range(12):
            plt.subplot(3, 4, i + 1)
            plt.plot(dataset.classes, GSDs_to_decompose[i] * 100, c="#ffffff00", marker=".", ms=3, mfc="k", mec="k")
            plt.plot(dataset.classes, GSDs[i] * 100, c="k")
            for j in range(dataset.n_components):
                plt.plot(dataset.classes, components[i, j] * proportions[i, 0, j] * 100, c=cmap(j))
            plt.xscale("log")
            plt.xlim(0.06, 3000)
            plt.ylim(0.0, 10.5)
            if i % 4 == 0:
                plt.ylabel("Frequency (%)")
                plt.yticks([2, 4, 6, 8], ["2", "4", "6", "8"])
            else:
                plt.yticks([2, 4, 6, 8], [])
            if i // 4 == 2:
                plt.xlabel(r"Grain size (μm)")
                plt.xticks([0.1, 1, 10, 100, 1000], ["0.1", "1", "10", "100", "1000"]),
            else:
                plt.xticks([0.1, 1, 10, 100, 1000], []),

        plt.tight_layout()
        plt.subplots_adjust(hspace=0.0, wspace=0.0)
        os.makedirs(frames_dir, exist_ok=True)
        plt.savefig(os.path.join(frames_dir, f"{frame_index}.png"), dpi=300.0)
        frame_index += 1
        # plt.pause(0.1)
        if frame_index > 300:
            break
